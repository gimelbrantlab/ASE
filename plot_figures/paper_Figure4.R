...
figure_4A <- ggplot(df_fig1A, 
       aes(method, DiffASE_N, col= method)) + 
  geom_boxplot() +
  theme_bw() +
  facet_grid(~ test) +
  scale_color_manual(values=c("gray","gray","gray", "royalblue1","maroon2","olivedrab3")) +
  ylab("Number of genes called differential") + 
  xlab("") +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())

figure_4B <- ggplot(rbind(res62_df_all,res61_df_all), aes(x=method, y=FP_rate, col=experiment)) +
  geom_boxplot() +
  xlab("") +
...
figure_4D <- ggplot() +
  ...
  scale_y_continuous(breaks=c(0:6), labels=10**c(0:6), limits = c(0, 6)) +
  scale_x_continuous(breaks=c(0:6), labels=10**c(0:6), limits = c(0, 6)) +
  facet_grid(what ~ .) +
  theme_bw() +
  ...
  theme(legend.position="None", text = element_text(size=18), axis.text.x = element_text(angle = 90)) +
  coord_fixed()
...
cowplot::save_plot(PLT_fig4, file="~/Dropbox (Partners HealthCare)/replicates_ASE/manuscript/Figures/fig.4_v1.pdf", base_height = 10, base_width = 16)
