...
PerformDiffAItest <- function(inDF, vect1CondReps_expA, vect1CondReps_expB, thr_coverage=NA, minDifference=0.1) {
  #'
  ...
  #' @return inDF table with 2 additional columns classifying genes into differentially allelically expressed (DAE) and not
  #' @examples
  #'
  ...
  df_tmp$deltaAI <- abs(df_tmp$meanAI1 - df_tmp$meanAI2)
  df_tmp$DAE <- F
  df_tmp$DAE[df_tmp$diffAI==T & df_tmp$deltaAI>=minDifference] <- T
  return(df_tmp)
}
...
DiffAIplusFig <- function(inDF, vect1CondReps_expA, vect1CondReps_expB, 
  ...
  #' @examples
  #' 
  df_tmp <- PerformDiffAItest(inDF, vect1CondReps_expA, vect1CondReps_expB, thr_coverage, minDifference)
  
  fig_compare_tmp <- ggplot(df_tmp, aes(x = meanAI1, y = meanAI2, col = DAE)) + 
    geom_point(size=0.5) + 
    theme_bw() +
